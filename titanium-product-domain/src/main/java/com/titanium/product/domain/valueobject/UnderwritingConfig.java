 ...
 * 核保配置值对象
 *
 * 定义该产品的核保策略。不同险种的核保要求差异巨大：
 * - 交强险/短期意外险：自动核保通过
 * - 标准寿险（保额<50万）：智核（规则引擎自动核保）
 * - 高保额寿险（保额≥50万）：人工核保
 * - 团险：自动核保 + 人工复核
 *
 ...
 */
public record UnderwritingConfig(
        ProductEnum.UnderwritingMode underwritingMode,
        String autoApprovalCondition,
        BigDecimal manualReviewAmountThreshold,
        List<String> requiredDocuments,
        Integer underwritingSLADays,

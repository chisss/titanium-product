 ...
 *
 * 核心设计思想：
 * - 当前Policy域硬编码了 Proposal→Insurance→Policy 三步出单
 * - 但实际业务中：
 *   · 车险（交强险）= 一步出单（录入即出单，无需核保）
 *   · 短期意外险/旅行险 = 两步出单（投保→保单，自动核保通过）
 *   · 寿险/重疾险 = 三步出单（意向单→投保单→核保→保单）
 *   · 团体寿险 = 三步出单 + 清单制（主保单+N个子保单）
 *   · 高保额财产险 = 三步 + 线下勘验（意向→勘验→投保→核保→保单）
 *
 * Product域通过此值对象定义该产品的出单步骤链路，Policy域消费此配置动态编排流程。
 *
...
public record IssuanceProcessConfig(
        ...
        List<ProductEnum.IssuanceStep> steps,
        boolean proposalRequired,
        boolean underwritingSkippable,

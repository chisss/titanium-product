...
/**
 * 保单形态配置值对象 定义该产品生成的保单结构形态： - 个单：一个投保人 → 一张保单 - 团单/清单制：一个主保单 → N个子保单（按被保险人拆分） -
 * 家庭单：一个主保单 → M个家庭成员子保单 - 联合保单：多个投保人共同持有一张保单 Policy域根据此配置决定生成保单时的结构。
 *
 ...
 */
public record PolicyFormConfig(ProductEnum.PolicyFormType policyFormType, boolean listPolicySupported,
                               Integer minSubPolicyCount, Integer maxSubPolicyCount,
                               boolean subPolicyIndependentEndorsement, List<ProductEnum.PolicyLevel> supportedLevels,
                               boolean beneficiaryRequired, Integer maxBeneficiaryCount) {
}
